...
import BINANCELogs from './App/BINANCELogs';
import { Button } from './components/button';

function BINANCE_Dashboard() {
  ...
  useEffect(() => {
    ...
    fetch(`${webapp}/calculate?exchange=binance`)
      .then(response => response.json())
      .then(data => {
        setMetrics(data);
      })
  ...
  const fetchLiquidityData = (country, paymentMethods) => {
    fetch(`${webapp}/get_liquidity?exchange=binance`, {
      method: 'POST',
      headers: {
  ...
  if (showLogs) {
    return <BINANCELogs exchangeName="BINANCE" />;
  }
...
export default BINANCE_Dashboard;